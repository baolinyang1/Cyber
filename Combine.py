...
# First part is from the Check.py
# import os
# import json
# import hashlib
# import mimetypes
...
# # if p.exists():
# #     #check if it's regular file(not a folder or a symlink)
# #     if p.is_file():
...
# Results["Magika-path"] = str(res.path)
# Results["Magika-status"] = res.status

# # Magika output prediction (final result)
# Results["Magika-prediction-label"] = res.prediction.output.label
# Results["Magika-prediction-mime_type"] = res.prediction.output.mime_type
# Results["Magika-prediction-group"] = res.prediction.output.group
# Results["Magika-prediction-description"] = res.prediction.output.description
...
#     if len(sys.argv) != 2:
#         print("useage is the script plus the argument")
#     else: 
#         if not os.path.exists(p):
#             print(f"Error:file '{p}' does not exist")
#         else:
#             if not p.is_file():
#                 print(f"Error:file '{p}' is not a regular file")
#             else:
...
if len(sys.argv) != 2:
    print("useage is the script plus the argument")
    # tell the following shii: no need to execute
    sys.exit()
else: 
    p = Path(sys.argv[1])
    if not os.path.exists(p):
...
Found_tools = dict()
# CHECK IF SYSTEM HAS these tools
Tools = ['md5sum',"sha1sum","sha224sum","sha256sum","sha384sum","sha512sum","rhash","openssl","ssdeep","exiftool", "sdhash"]
def CheckSysHash(tools):
    for hash in tools:
        try:
